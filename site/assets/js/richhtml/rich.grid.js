...
RichHTML.grid = function(config){
   ...
   this.width= '100%';
   this.tableclass = '';
   this.internalTpl = "<div class='richtable'><table id='{rich-id}' style='{tablestyle}' class='{tableclass}'><thead><tr>{{#columns}}<th nowrap='nowrap' rowspan='1' colspan='1' class='{{class}} {{align}} {{sortable}} {{xtype}}' {{#getsortfieldname}}{{sortFieldName}}{{/getsortfieldname}} dataindex='{{dataIndex}}' style='width:{{width}};{{#hidecolumn}}{{hidden}}{{/hidecolumn}}'><span class='{{sort_icon_class}}'>{{text}}&nbsp;</span></th>{{/columns}}</tr></thead><tbody>{tbody}</tbody></table><div class='richgrid-pagenavi-wrapper'></div></div>";
   ...
   if (config.metaEl) {
      this.metaEl = config.metaEl;
   }
   else {
      this.metaEl = this.el+"-metadata";
   }
   ...
   if (config.baseParams) {
      jQuery.extend(this.baseParams, config.baseParams);
   }
   if (config.totalProperty) {
      this.totalProperty = config.totalProperty;
   }
      ...
      if (groups.hasOwnProperty(key)) {
         ...
         json.groups.push({
            ...
            groupname:{
               ...
               name:obj[0][this.groupField],
               cols:json.columns.length
               }
            });
   }
   }
      ...
      $.getJSON(self.url, self.baseParams, function(data) {
         ...
         if(data != null && data.totalCount != 0) {
            ...
            self.onLoad(true);
               ...
               $('#tablemetadata').html("Displaying tickets 0 - 0 of 0");  
               $('#'+self.el+' tbody').html('<tr><td class="richgrid-nodata" colspan="'+json.columns.length+'">'+self.emptyText+'</td></tr>');  
         }
      });
   } else {
...
RichHTML.grid.prototype.mapCellModifiers = function (el,id) {
   var self = this;
   if(typeof(el.addClass)!== "undefined") {
      $('td[id="'+id+'"]').addClass(el.addClass);
   }
   if(typeof(el.removeClass)!== "undefined") {
      $('td[id="'+id+'"]').removeClass(el.removeClass);
   }
   if(typeof(el.addStyle)!== "undefined") {
      $('td[id="'+id+'"]').attr('style',addStyle);
   }
   if(typeof(el.removeStyle)!== "undefined" && el.removeStyle) {
      $('td[id="'+id+'"]').attr('style','');
   }
   $(self).unbind('colload');
};
...
RichHTML.grid.prototype.addRenderers = function (cols,rows) {
   ...
   $.each(cols,function(c_i, c_val) {
      if ((typeof(c_val.dataIndex) === "undefined") && (typeof(c_val.renderer) === "undefined")) {
         if (c_val.xtype !== "drag") {
            RichHTML.debug(1,"Error: You need either a dataIndex or renderer for column:"+c_i);
         }
      }
...
RichHTML.grid.prototype.templatePrep = function()
{
   ...
   $.each(self.columns,function(i, val) {
      ...
      if (val.xtype === "expander") {
         ...
         hiddenstyle = (val.hidden) ? "visibility:hidden;display:none" : "";
         ...
         if(typeof(val.width) !== "undefined") {
            ...
            if( (widthStr.indexOf("px") != -1) || (widthStr === "100%") ){
               ...
               self.columns[i].width = val.width+"px";
            }
         } else if (!val.hidden){
         ...
         itemIndex = (typeof(val.escapeHTML)==="undefined" || !val.escapeHTML) ? "{"+val.dataIndex+"}" : val.dataIndex;
         cols += "<td id='{{#cellid}}{{/cellid}}' data-title=" + val.text + " style='overflow: visible;"+hiddenstyle+"' class='"+((typeof(val.class) !== "undefined") ? val.class : "")+ " " + ( (typeof(val.align) !== "undefined") ? val.align : "left")+"' "+richid+"  dataindex='"+val.dataIndex+"' valign='top'>{{#"+colCount+"-rich-renderer}}{{#getlastcellid}}{{/getlastcellid}}{||}{{"+itemIndex+"}}{{/"+colCount+"-rich-renderer}}</td>";
      }
      colCount++;
   });
   ...
   self.internalTpl = RichHTML.replaceAll(self.internalTpl,"{tableclass}",this.tableclass);
...
RichHTML.grid.prototype.dataPrep = function()
{
   ...
   json.getsortfieldname = function (){
      return function (text, render) {
         return (text) ? "sortfieldname='"+text+"'": "";
      };
   
};
json.checkboxid = function (){
   return function (text, render) {
      return self.id+"-cell-"+(++cellCounter)+"{-}"+text;
   };

};
json.expanderid = function (){
   return function (text, render) {
      return self.id+"-expander-"+(++expanderCounter);
   };

};
json.cellid = function (){
   return function (text, render) {
      return self.id+"-cell-"+(++cellCounter);
   };

};
json.getlastcellid = function (){
   return function (text, render) {
      return cellCounter;
   };
...
json.getcollapsedstate = function (){
   return function (text, render) {
...
json.getrowid = function (){
   return function (text, render) {
      return self.id+"-row-"+(++rowCounter);
   };

};
json.getlastextenderid = function (){
   return function (text, render) {
      return expanderCounter;
   };

};
json.expanderrowid = function (){
   return function (text, render) {
      return self.id+"-expander-"+(expanderCounter)+"-data";
   };
...
RichHTML.grid.prototype.onLoad = function (reloading) {
   ...
   if (!self.data) {
      ...
      if (self.pagingEl !== null){
         ...
         $("#"+self.id).parent().children(".richgrid-pagenavi-wrapper").html("");
      }
   } else {
      ...
      if (self.hasCheckbox) {
         $("#"+self.id+" tbody td .checkbox-icon").bind('click', function(event) {
            //trigger rowselect
            var rowid, data;
            if(typeof(event.massclick)==="undefined") {
               event.massclick = false;
            }
         ...
         // Handle row click
         $("#"+self.id+" tbody td").click(function(event) {
             
             // DOn't do anything if we're clicking a link or the checkbox
            if( ! $(event.target).is('a')) {
                if($(event.target).attr('class') != 'checkbox-icon' && $(event.target).attr('class') != 'checkbox-icon checkbox-icon-checked') {
                
                    // Check the box
                    $('#' + this.parentNode.id).children('td:first').children('div:first').toggleClass('checkbox-icon-checked');

                    // highlight the row
                    $('#' + this.parentNode.id).toggleClass('row-selected');
                    
                    // Handle the buttons
                    if(self.getSelectedRowIds().length >= 1) {
                        $("#ticketGrid-footer :input").attr("disabled", false);

                        // We can't merge 1 ticket
                        if(self.getSelectedRowIds().length == 1) {
                        $("#btnMergeTicket").attr("disabled", true);
                        }
                    } else {
                        $("#ticketGrid-footer :input").attr("disabled", true);
                    }
                }
            }
        });
      ...
      if (self.pagingData.totalItems && self.pagingData.totalItems !== null) {
         ...
         {
            if (self.pagingData.page === i) {
               buttonshtml += "<span class='current'>"+i+"</span>";
            }
            else {
               buttonshtml += "<span data-"+self.id+"-link='"+((i-1)*self.baseParams.limit)+"'>"+i+"</span>";
            }
         }
         ...
         $("*[data-"+self.id+"-link]").bind("click",function(){
            self.reload({
               params:{
                  start:$(this).attr("data-"+self.id+"-link")
                  }
               });
         });
...
RichHTML.grid.prototype.setMetaData = function() {
   ...
   if (typeof(self.metaEl) !== "undefined") {
      $('#'+self.metaEl).html(meta);
   }
   return meta;
};
