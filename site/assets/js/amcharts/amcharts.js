    ...
    draw: function () {
        ...
        if (this.logarithmic && (0 >= this.getMin(0, this.data.length -
            ...
            if (isNaN(this.min) || isNaN(this.max) || !this.foundGraphs || Infinity == this.min || -Infinity == this.max) g = !0;
                ...
                // HACK to get the steps be minimum 1 satoshi
                if (this.step < 0.00000001) {
                    this.step = 0.00000001;
                }
                if (this.max < 0.00000005) {
                    this.min = Number(this.min - 0.00000001).toFixed(8);
                    this.max = Number(this.max + 0.00000001).toFixed(8);
                }
                // END HACK
...
AmCharts.RecItem = AmCharts.Class({
    construct: function (a, b, c, d, e, f, g, h, k, l) {
        ...
        y && (y.attr({
            "text-anchor": P
        }), y.translate(G, E), 0 !== B && y.rotate(-B, a.chart.backgroundColor), a.allLabels.push(y), " " != c && (this.label = y));
...
AmCharts.AmGraph = AmCharts.Class({
    ...
    createSerialGraph: function () {
        ...
        var l = this.width,
            ...
            u = [],
            v = [],
            t, w, x, F, z = this.chart.graphs.length,
            A, I = this.dx / this.depthCount,
            K = this.dy / this.depthCount,
            B = f.stackType,
            J = this.labelPosition,
            M = this.start,
            ca = this.end,
            U = this.scrollbar,
        ...
        if ("line" == g || "step" == g || "smoothedLine" == g) {
            if (0 < M) {
                for (H = M - 1; - 1 < H; H--)
                    if (ha = d[H], y = ha.axes[f.id].graphs[b], Wa = y.values.value, !isNaN(Wa)) {
                        M = H;
                        break
                    }
                if (this.lineColorField)
                    for (H = M; - 1 < H; H--)
                        if (ha = d[H], y = ha.axes[f.id].graphs[b],
                            y.lineColor) {
                ...
                if (this.fillColorsField)
                    for (H = M; - 1 < H; H--)
                        if (ha = d[H], y = ha.axes[f.id].graphs[b], y.fillColors) {
                            this.fillColorsSwitched = y.fillColors;
                            break
                        }
                if (this.dashLengthField)
                    for (H = M; - 1 < H; H--)
                        if (ha = d[H], y = ha.axes[f.id].graphs[b], !isNaN(y.dashLength)) {
                            this.dashLengthSwitched = y.dashLength;
                            break
                        }
            }
            if (ca < d.length - 1)
                for (H = ca + 1; H < d.length; H++)
                    if (ha = d[H], y = ha.axes[f.id].graphs[b], Wa = y.values.value, !isNaN(Wa)) {
                        ca = H;
                        break
                    }
        ...
        for (H = M; H <= ca; H++) {
            ...
            if (ua) {
                ...
                var ia = ha.x[Na.id],
                    ...
                    la = Math.floor(T / 2) + Math.floor(Ka * T / 2),
                    va = la,
                    Ab = 0;
                        ...
                        if (!(Z < Bb && na < Bb || Z > Cb && na > Cb))
                            if (n) {
                                ...
                                aa = AmCharts.fitToBounds(aa, -Q, l - Q + I *
                                    S);
                                if (R < m && 0 < O && (qa = new AmCharts.Cuboid(e, aa, O, I - a.d3x, K - a.d3y, Ca, Ra, V, oa, $, G, q, n, P, Sa), "bottom" != J && "inside" != J && "middle" != J))
                                    if (J = f.reversed ? "left" : "right", 0 > Z) J = f.reversed ? "right" : "left";
                                ...
                                if (Q < l + S * I && 0 < O)
                                    if (qa = new AmCharts.Cuboid(e,
                                        O, aa, I - a.d3x, K - a.d3y, Ca, Ra, V, oa, this.lineAlpha, G, q, n, P, Sa), 0 > Z && "middle" != J && "inside" != J) J = "bottom";
                                    else if (J = pa, "regular" == B || "100%" == B) fa += this.dy
                            }
                ...
                case "candlestick":
                    if (!isNaN(na) && !isNaN(Z)) {
                        ...
                        if (n) {
                            ...
                            var qa = new AmCharts.Cuboid(e, O, aa, I, K, Ca, Ra, V, oa, $, G, q, n, P, Sa),
                                pb, qb;
                ...
                if (!U && !isNaN(Z)) {
                    ...
                    if (this.end - this.start <= Fb || 0 === Fb) {
                        ...
                        if (Gb) {
                            var ea = this.createLabel(y, 0, 0, Gb),
                                wa = 0,
                                xa = 0,
                                Hb = ea.getBBox(),
                                La = Hb.width,
    ...
    drawLineGraph: function (a, b, c, d) {
        ...
        if (1 < a.length) {
            ...
            if (0 < f || 0 < v)
                if (m = a.join(";").split(";"), n = b.join(";").split(";"), "serial" == e.chart.type && (0 < c.length ? (c.reverse(), d.reverse(), m = a.concat(c), n = b.concat(d)) : e.rotate ? (n.push(n[n.length - 1]), m.push(t), n.push(n[0]), m.push(t), n.push(n[0]), m.push(m[0])) : (m.push(m[m.length -
                ...
                if (c.mouseIsOver || A || I || this.forceShow)
                    if (this.forceShow = !1, I) {
                        ...
                        0 !== e && (h = {
                            type: "zoomed",
                            target: this
                        }, h.chart = this.chart, g.parseDates && !g.equalSpacing ? (c + e > x && (e = x - c), A + e < w && (e = w - A), h.start = A + e, h.end = c + e, this.fire(h.type, h)) : J + e >= this.data.length || 0 > l + e || (h.start = l + e, h.end = J + e, this.fire(h.type, h)))
                    } else {
                        ...
                        if (r) {
                            ...
                            0 < v && !this.zooming ? isNaN(this.previousX) ? M.translate(w, x) : (M.translate(this.previousX, this.previousY), M.animate({
                                translate: w +
                                    "," + x
                            }, v, "easeOutSine")) : M.translate(w, x);
                            this.previousX = w;
                            this.previousY = x;
                            M.show()
                        }
                        ...
                        if (a != this.previousIndex || J != this.previousMostCloseGraph)
                            if (this.normalizeBulletSize(), this.destroyValueBalloons(), this.resizedBullets = [], K && this.valueBalloonsEnabled && v && c.balloon.enabled) {
                                this.valueBalloons = v = [];
                                for (t = 0; t < K.length; t++)
                                    if (g = K[t], (!this.oneBalloonOnly || g == J) && g.showBalloon && !g.hidden && g.balloonText && ("step" == g.type && "left" == g.stepDirection &&
                                        (s = this.data[a + 1]), s)) {
                                        ...
                                        if (this.showNextAvailable && isNaN(x) && a + 1 < this.data.length)
                                            for (k = a + 1; k < this.data.length; k++)
                                                if (q = this.data[k])
                                                    if (w = q.axes[g.valueAxis.id].graphs[g.id], x = w.y, !isNaN(x)) break;
                                        if (!isNaN(x)) {
                                            q = w.x;
                                            u = !0;
                                            if (r) {
                        ...
                        b ? (h = {
                                type: "changed"
                            }, h.index = a, h.chart = this.chart, h.zooming = A, h.mostCloseGraph = J, h.position =
...
AmCharts.wedge = function (a, b, c, d, e, f, g, h, k, l, m, p) {
    ...
    var u = 1 / 180 * Math.PI,
        ...
        t = b + Math.sin(d * u) * f,
        w = c - Math.cos(d * u) * g,
        x = b + Math.sin((d + e) * u) * f,
        F = c - Math.cos((d + e) * u) * g,
